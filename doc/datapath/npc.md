...
若 `jump_mode == NPC_JUMP_WHEN_EQUAL`，则 `alu_comp_result == ALU_EQUAL` 时，首先把 `num` 扩展成 32 位有符号立即数，然后令 `next_pc = $signed(curr_pc) + $signed(num)`。否则做跟 `jump_mode == NPC_JUMP_DISABLE` 时相同的步骤。
...
2. TODO: 把 curr\_pc 看成有符号数，太大了不行。
